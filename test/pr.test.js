...
describe('pr', () => {
  ...
  it('infers parent branch and crafts a PR URL', async () => {
    ...
    assert.match(url.pathname, /compare\/master\.\.\.jdoe\/feature\/master\/kittens-are-cute/, 'path has the proper comparison');
    assert.strictEqual(url.query.title, 'added a kitten', 'title has nlm-prefix-stripped commit msg');
    assert.match(url.query.body, /some msg/, 'body has the commit msg body');
  });
  it('uses branch name as title for multi-commit PRs', async () => {
    const url = await setupForPR(t, ['fix: added kitten 1', 'fix: added kitten 2']);
    assert.strictEqual(url.query.title, 'kittens are cute', 'title has cleaned up branch name');
    assert.match(url.query.body, /\* fix: added kitten 1
\* fix: added kitten 2
/, 'body has list of commit subjects');
  ...
  it('respects the contents of a PULL_REQUEST_TEMPLATE', async () => {
    ...
    assert.match(url.query.body, /Important stuff/, 'contents of a PULL_REQUEST_TEMPLATE.md file');
  });
  it('respects the contents of a .github/pull_request_template', async () => {
    ...
    const url = await setupForPR(t, ['feat: use a PR template']);
    assert.match(url.query.body, /Other stuff/, 'contents of a pull_request_template.md file');
  });
  it('optionally ignores PULL_REQUEST_TEMPLATE', async () => {
    ...
    assert.ok(!url.query.body.includes('Important stuff'), 'contents of the PULL REQUEST TEMPLATE are not present');
  });
  it('uses nlm prefixes to pick a title', async () => {
    const url = await setupForPR(t, ['feat: the important bit', 'docs: added kitten 2']);
    assert.strictEqual(url.query.title, 'the important bit', 'title has important subject');
  });
  it('dies with pretty error on out-of-date push', async () => {
    ...
    await Promise.all([t.git.commit('one', ['README']), t.git2.commit('two', ['README'])]);
    await t.git2.push();
    await assert.rejects(prAction({
      deps: t,
  ...
  describe('on a feature branch on a fork', () => {
    ...
    it('infers parent branch and crafts a PR URL from fork', async () => {
      ...
      assert.ok(url.pathname.includes(`compare/master...${prefix}:feature/master/kittens-are-cute`), 'path has the proper comparison');
      assert.match(url.query.body, /https:\/\/github\.com\/groupon\/git-workflow\/releases\/tag\/v/, 'body has the footer url');
    });
  });
});
